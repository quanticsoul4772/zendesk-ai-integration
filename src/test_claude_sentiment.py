"""
Test script for Claude sentiment analysis.

This script tests Claude's sentiment analysis capabilities with different sample tickets
...
from claude_service import analyze_ticket_content
from claude_enhanced_sentiment import enhanced_analyze_ticket_content
...
def display_analysis_results(title: str, analysis: Dict[str, Any]) -> None:
    ...
    # Print key phrases
    key_phrases = sentiment.get("key_phrases", []) if isinstance(sentiment, dict) else []
    if key_phrases:
        print("Key Phrases:")
...
def main():
    """Test Claude's sentiment analysis with sample tickets."""
    print("\nTesting Claude Sentiment Analysis\n")
    ...
    for i, sample in enumerate(SAMPLE_TICKETS, 1):
        ...
        analysis = analyze_ticket_content(content)
        display_analysis_results(f"{title} (Basic Analysis)", analysis)
    ...
    for i, sample in enumerate(SAMPLE_TICKETS, 1):
        ...
        display_analysis_results(f"{title} (Enhanced Analysis)", analysis)
    
    print("Claude sentiment analysis testing complete!")
